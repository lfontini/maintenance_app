...
![Image title](/media/close.png)

![Image title](/media/close01.png)

![Image title](/media/close02.png)


## Overview

The code includes functions to generate a notification template for closing tickets and to interact with the Zendesk API to close the tickets.

## Functions

### `generate_notification_template()`

This function generates an HTML template for the notification sent when closing tickets.

### `make_request_zendesk(method, url, body)`

This function makes a request to the Zendesk API.
...
This function closes tickets in Zendesk.

- `tickets` (list): List of ticket IDs to be closed.

Returns the ID of the closed ticket(s) or `None` if no tickets are found.
...
def generate_notification_template():
    # html template
    '''
    This function return a template with a close ticket comunication 
